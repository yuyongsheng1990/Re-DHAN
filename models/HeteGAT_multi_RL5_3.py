...
# @Description :
"""
    GATConv with 3-layer edge_index + residual connection: l0*h0+l1*h1+l2*h2
    para l refer to: Neural Networks 2022-Augmented Graph Neural Network with hierarchical global-based residual connection.
"""
...
class GraphFormer(nn.Module):
    ...
    def __init__(self, in_dim, hid_dim, out_dim, heads) -> None:  # in_dim, 302; hid_dim, 128; out_dim, 64, heads, 4
        ...
        self.GraphFormer1 = GATConv(in_channels=in_dim, out_channels=hid_dim, heads=heads, add_self_loops=False)  # 这只是__init__函数声明变量
        self.GraphFormer2 = GATConv(in_channels=hid_dim * heads, out_channels=hid_dim, heads=heads, add_self_loops=False)  # 隐藏层维度，输出维度64
        self.GraphFormer3 = GATConv(in_channels=hid_dim * heads, out_channels=hid_dim, heads=heads, add_self_loops=False)  # 隐藏层维度，输出维度64
        ...
        # previous node embeddings for residual connection

        self.hid_dim = hid_dim
        self.heads = heads

        # coefficient parameters for h0, h1, and h2
        self.coff_a = nn.Parameter(torch.Tensor(1, 1))
        self.coff_t = nn.Parameter(torch.Tensor(1, 1))
    ...
    def forward(self, x, adjs, device):  # 这里的x是指batch node feature embedding， adjs是指RL_samplers 采样的batch node 子图 edge
        for i, (edge_index, _, size) in enumerate(adjs):  # adjs list包含了从第L层到第1层采样的结果，adjs中子图是从大到小的。adjs(edge_index,e_id,size), edge_index是子图中的边
            ...
            # # my designed residual connection, and set TransformerConv para root_weight=False
            coff = torch.pow(input=self.coff_a, exponent=(self.coff_t+i))
            x += x_target * coff
            if i <= 1:
...
# HAN model with RL
class HeteGAT_multi_RL5_3(nn.Module):
    '''
    ...
    def __init__(self, feature_size, nb_classes, nb_nodes, attn_drop, feat_drop, hid_dim, out_dim, time_lambda,
        ...
        super(HeteGAT_multi_RL5_3, self).__init__()
        ...
        self.mlp = nn.Sequential(
                    ...
                    ELU(inplace=True),
                    Dropout(),
                    Linear(self.hid_dim, self.hid_dim * self.n_heads[0],  bias=True),)

        # 2层 GAT module - 2 from FinEvent 300,
        # hid_dim=128, out_dim=64, heads=4
        # self.intra_aggs = nn.ModuleList([GraphFormer(self.hid_dim, int(self.hid_dim/2), self.out_dim, self.n_heads[0])
        #                                  for _ in range(self.num_relations)])   # DHAN-4: original version
        self.intra_aggs = nn.ModuleList([GraphFormer(self.hid_dim * self.n_heads[0], self.hid_dim, self.out_dim, self.n_heads[0])
                                         for _ in range(self.num_relations)])  # DHAN-4: original version

        # ---------------------Final Linear module-------------------------------
        # self.final_linear = nn.Linear(out_dim, out_dim, bias=True, weight_initializer='glorot')
        self.final_linear = nn.Linear(self.hid_dim * self.n_heads[0], self.out_dim, bias=True)
