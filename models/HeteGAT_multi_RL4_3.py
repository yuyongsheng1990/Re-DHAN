...
class GraphFormer(nn.Module):
    ...
    def __init__(self, in_dim, hid_dim, out_dim, heads) -> None:  # in_dim, 302; hid_dim, 128; out_dim, 64, heads, 4
        ...
        self.GraphFormer1 = TransformerConv(in_channels=in_dim, out_channels=hid_dim, heads=heads, dropout=0.2, root_weight=False)  # 这只是__init__函数声明变量
        self.GraphFormer2 = TransformerConv(in_channels=hid_dim * heads, out_channels=hid_dim, dropout=0.2, root_weight=False)  # 隐藏层维度，输出维度64
        self.GraphFormer3 = TransformerConv(in_channels=hid_dim, out_channels=out_dim, dropout=0.2, root_weight=False)  # 隐藏层维度，输出维度64
        ...
        self.lnr1 = Linear(in_features=in_dim, out_features=hid_dim * heads)
        self.lnr2 = Linear(in_features=hid_dim * heads, out_features=hid_dim)
        self.lnr3 = Linear(in_features=hid_dim, out_features=out_dim)
        self.lnr_list = ModuleList([self.lnr1, self.lnr2, self.lnr3])
        # decay weights for residual connection
        self.decay_weight = nn.Parameter(torch.Tensor(3, 1))
    ...
    def forward(self, x, adjs, device):  # 这里的x是指batch node feature embedding， adjs是指RL_samplers 采样的batch node 子图 edge
        for i, (edge_index, _, size) in enumerate(adjs):  # adjs list包含了从第L层到第1层采样的结果，adjs中子图是从大到小的。adjs(edge_index,e_id,size), edge_index是子图中的边
            ...
            x = self.decay_weight[i] * x_r + (1-self.decay_weight[i]) * x + self.decay_bias[i]
            if i == 0:
...
# HAN model with RL
class HeteGAT_multi_RL4_3(nn.Module):
    '''
    ...
    def __init__(self, feature_size, nb_classes, nb_nodes, attn_drop, feat_drop, hid_dim, out_dim, time_lambda,
        ...
        super(HeteGAT_multi_RL4_3, self).__init__()
